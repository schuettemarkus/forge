...
import { useOpportunities, useScrapeAndScore } from "@/lib/hooks";
...
export default function OpportunitiesPage() {
  ...
  const scrape = useScrapeAndScore();
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6 lg:mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          <RefreshCw className={`mr-2 h-3.5 w-3.5 ${scrape.isPending ? "animate-spin" : ""}`} />
          {scrape.isPending ? "Scanning..." : "Scan Trends"}
        </Button>
      </div>

      {error && (
        <div className="mb-4 glass rounded-xl p-3 text-sm text-warning border border-warning/20">
          API unavailable — start the backend with: make api
        </div>
      )}

      {scrape.isError && (
        <div className="mb-4 glass rounded-xl p-3 text-sm text-destructive border border-destructive/20">
          Scrape failed — is the backend running?
        </div>
      )}
